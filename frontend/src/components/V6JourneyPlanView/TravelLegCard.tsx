...
import { ArrowRight, Calendar, DollarSign, Info, Route, Clock, Repeat } from 'lucide-react';
  ...
  return (
    <div className="flex gap-4 py-2">
      ...
      <div className="flex flex-col items-center flex-shrink-0 w-12">
        <div className="w-0.5 flex-1 bg-[#E8E0D4]" />
      </div>

      {/* Travel info */}
      <div className="flex-1 min-w-0">
        <div className="bg-[#FDF8F3] border border-[#E8E0D4] rounded-2xl p-4 flex items-center gap-4">
          {/* Transport icon */}
          <div
            className="flex-shrink-0 w-12 h-12 rounded-full flex items-center justify-center text-white shadow-md bg-[#D4A574]"
          >
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 text-sm font-display font-semibold text-gray-900">
              ...
              <ArrowRight className="h-3 w-3 text-[#9C8E82] flex-shrink-0" />
              <span className="truncate">{leg.to_city}</span>
            </div>
            <div className="flex items-center gap-3 mt-1 text-xs text-gray-600 flex-wrap">
              {travelDate && (
                <span className="flex items-center gap-1 font-semibold text-[#A66244] bg-primary-50 px-2 py-0.5 rounded-full">
                  <Calendar className="h-3 w-3" />
                  {formatTravelDate(travelDate)}
                </span>
              )}
              {leg.mode && (
                <span className="capitalize font-bold text-[#8B9E6B] bg-[#F5F7F0] px-2 py-0.5 rounded-full">{leg.mode}</span>
              )}
              {hasValidDuration && (
                <>
        ...
        {/* Departure/arrival times + transfers */}
        {(leg.departure_time || (leg.num_transfers && leg.num_transfers > 0)) && (
          <div className="mt-2 flex items-center gap-3 text-xs text-gray-600 px-2 flex-wrap">
            {leg.departure_time && leg.arrival_time && (
              <span className="flex items-center gap-1 bg-[#F7F5F2] text-[#6E655B] px-2 py-0.5 rounded-full font-medium">
                <Clock className="h-3 w-3" />
                {leg.departure_time} → {leg.arrival_time}
              </span>
            )}
            {leg.num_transfers != null && leg.num_transfers > 0 && (
              <span className="flex items-center gap-1 bg-amber-50 text-amber-700 px-2 py-0.5 rounded-full font-medium">
                <Repeat className="h-3 w-3" />
                {leg.num_transfers} transfer{leg.num_transfers > 1 ? 's' : ''}
              </span>
            )}
          </div>
        )}
        ...
        {(leg.booking_tip || leg.notes) && (
          <div className="mt-2 flex items-start gap-2 text-xs text-gray-500 px-2 bg-gray-50/80 rounded-xl py-2">
            <Info className="h-3 w-3 flex-shrink-0 mt-0.5 text-[#9C8E82]" />
...
export const TravelLegCompact = memo(function TravelLegCompact({ leg, palette }: TravelLegCompactProps) {
  ...
  // Default to warm earth tone if no palette provided
  ...
  const bgStyle = palette
    ...
    : 'linear-gradient(to right, #FDF8F3, #FEFCFA, #FDF8F3)';
  const borderColor = palette?.borderColor || '#E8E0D4';
  const accentColor = palette?.textColor || '#A66244';
        ...
        {leg.mode && (
          <>
            ...
            <span 
              ...
              style={{ backgroundColor: palette?.bgColor || '#FDF8F3', color: accentColor }}
            >
              {leg.mode}
            </span>
          </>

...
export function ItineraryInputForm({ onSubmit, loading, error }: ItineraryInputFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      {/* Error Banner */}
      {error && (
        <div className="bg-red-50/80 border border-red-200/60 rounded-2xl p-4 flex items-start gap-3 animate-scale-in">
          <AlertCircle className="h-5 w-5 text-red-500 flex-shrink-0 mt-0.5" />
          <div>
            <p className="text-sm font-display font-semibold text-red-800">Generation Error</p>
      ...
      <div className="bg-white rounded-2xl shadow-md overflow-hidden border border-gray-100/60 transition-all duration-300 hover:shadow-lg">
        ...
        <div className="p-5">
          <input
            ...
            className="w-full px-4 py-3 border border-gray-200/80 rounded-xl focus:ring-2 focus:ring-primary-400/30 focus:border-primary-400 transition-all text-lg bg-gray-50/50 focus:bg-white"
            disabled={loading}
          />
          <p className="text-xs text-gray-400 mt-2">Enter a city and country for best results</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-md overflow-hidden border border-gray-100/60 transition-all duration-300 hover:shadow-lg">
        ...
        >
          <div className="flex items-center justify-between">
            ...
            <span className="text-sm opacity-90 font-medium">{selectedInterests.length}/5 selected</span>
          </div>
        </div>
        <div className="p-5">
          ...
          {selectedInterests.length === 0 && (
            <p className="text-xs text-gray-400 mt-3">Select 1-5 interests to personalize your itinerary</p>
          )}
        </div>
      </div>
            ...
            {(['relaxed', 'moderate', 'packed'] as const).map((p) => (
              <button
                ...
                className={`p-4 rounded-xl border-2 text-center transition-all duration-200 ${
                  pace === p
                    ? 'border-primary-400 bg-primary-50/50 text-primary-700 shadow-sm'
                    : 'border-gray-200/80 hover:border-gray-300 bg-gray-50/30'
                } disabled:opacity-50`}
              >
                <span className="text-2xl mb-1 block">
                  {p === 'relaxed' ? '🧘' : p === 'moderate' ? '🚶' : '🏃'}
                </span>
                <span className="text-sm font-display font-semibold capitalize">{p}</span>

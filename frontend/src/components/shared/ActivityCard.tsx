/**
 * Shared ActivityCard - Single activity display within a day plan
 * Used by both V6JourneyPlanView and ItineraryView
 */
import { memo } from 'react';
import { Clock, MapPin, Star, Navigation, ExternalLink } from 'lucide-react';
import { categoryStyles, getCategoryIcon, isMealCategory } from '@/styles';
import { formatDuration } from '@/components/shared/utils';

/** Common activity shape used by the card (subset of both V6Activity and Activity) */
...
export const ActivityCard = memo(function ActivityCard({ activity, isLast, index }: ActivityCardProps) {
  ...
  const isMeal = isMealCategory(category);
  ...
  return (
    ...
    >
      ...
      <div className={`flex-1 min-w-0 ${isLast ? '' : 'pb-4'}`}>
        <div
          className="rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-all duration-300 group-hover:translate-x-0.5"
          style={{
            background: 'white',
          ...
          <div className="p-4">
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                <h5 className="font-display font-semibold text-gray-900 text-base leading-tight line-clamp-2">
                  {activity.place.name}
                </h5>
                ...
                <div className="flex items-center gap-2 mt-2 flex-wrap">
                  ...
                  <span className="text-xs text-gray-500 flex items-center gap-1.5 font-medium bg-gray-50/80 px-2 py-1 rounded-full">
                    <Clock className="h-3 w-3" />
                    {formatDuration(activity.duration_minutes)}
                  </span>
                  {activity.place.rating && (
                    <span className="text-xs text-gray-600 flex items-center gap-1 font-medium bg-amber-50/80 px-2 py-1 rounded-full">

...
- **Orchestrators** (`app/orchestrators/`): Pipeline coordination
  - `journey.py` — JourneyOrchestrator: Landscape Discovery(Google, categorized) -> Scout(LLM, experience_themes not highlights) -> Enrich(Google APIs) -> Review(LLM, score>=75?) -> Planner(LLM, fix issues) -> loop (tracks best plan across iterations, max 3)

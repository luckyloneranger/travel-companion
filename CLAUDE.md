...
- **Agents** (`app/agents/`): LLM-powered components
  ...
  - Day plan pipeline: `day_scout.py` (batch activity selection for themed days from candidates + landmarks), `day_reviewer.py` (LLM scores 7 dimensions: theme coverage, landmarks, variety, duration, pacing, meals, activity count), `day_fixer.py` (LLM fixes reviewer issues by swapping activities), `day_planner.py` (legacy single-shot fallback for stored trips without experience_themes)

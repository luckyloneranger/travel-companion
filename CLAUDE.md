...
Unified pipeline: multi-city journey planning with Landscape Discovery (Google) -> Scout -> Enrich -> Review -> Planner loop (~2-5min), iterating until quality threshold (min 70 score, max 3 iterations, returns best attempt). Day plans are generated in background per-city with batched quality pipeline: theme pre-mapping -> Day Scout -> Day Reviewer -> Day Fixer loop (max 2 iterations per batch of 2-3 days) -> TSP optimize -> schedule -> route computation -> weather integration. All trip endpoints require authentication.
...
- **Orchestrators** (`app/orchestrators/`): Pipeline coordination
  - `journey.py` — JourneyOrchestrator: Landscape Discovery(Google, categorized) -> Scout(LLM, experience_themes not highlights) -> Enrich(Google APIs) -> Review(LLM, score>=70?) -> Planner(LLM, fix issues) -> loop (tracks best plan across iterations, max 3)
  - `day_plan.py` — DayPlanOrchestrator: cities processed in parallel (Queue + Semaphore, bounded by `MAX_CONCURRENT_CITIES`). Per city: theme-to-day pre-mapping -> parallel landmark discovery (asyncio.gather) -> batched Day Scout(LLM) -> Day Reviewer(LLM, 7 dimensions) -> Day Fixer(LLM) -> loop (max 2 per batch of 2-3 days) -> TSP optimize -> schedule (culture-aware meal placement) -> pace-aware transport mode selection -> route computation -> graduated weather warnings. Excursion days also processed in parallel via gather. Failed cities retry once then emit `city_error` SSE event.
- **Agents** (`app/agents/`): LLM-powered components
  - Journey pipeline: `scout.py` (city selection + experience_themes + accommodation, city-state collapse), `enricher.py` (Google API grounding, segment-based transport, ferry guard, accommodation.why preservation), `reviewer.py` (quality scoring 0-100, experience theme + landscape validation), `planner.py` (fix review issues with theme context)
  - Day plan pipeline: `day_scout.py` (batch activity selection for themed days from candidates + landmarks, filters lodging types), `day_reviewer.py` (LLM scores 7 dimensions: theme coverage, landmarks, variety, duration, pacing, meals, activity count), `day_fixer.py` (LLM fixes reviewer issues by swapping activities, duplicate-aware via `already_used`), `day_planner.py` (legacy single-shot fallback for stored trips without experience_themes)
...
- **Config** (`app/config/`): Settings (Pydantic BaseSettings), planning configs (`planning.py` — pace configs, fallback duration-by-type table, interest-to-place-type seed mapping, adaptive place filters, haversine fallback computation, `LODGING_TYPES` filter set), regional transport guidance (`regional_transport.py` — LLM-driven prompt guidance instead of hardcoded profiles)

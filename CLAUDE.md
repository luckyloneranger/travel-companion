...
- **Orchestrators** (`app/orchestrators/`): Pipeline coordination
  ...
  - `day_plan.py` — DayPlanOrchestrator: theme-to-day pre-mapping -> per-city landmark discovery -> batched Day Scout(LLM) -> Day Reviewer(LLM, 7 dimensions) -> Day Fixer(LLM) -> loop (max 2 per batch of 2-3 days) -> TSP optimize -> schedule (culture-aware meal placement) -> pace-aware transport mode selection -> route computation -> graduated weather warnings per city
